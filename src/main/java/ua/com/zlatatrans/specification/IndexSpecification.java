...
public class IndexSpecification implements Specification<Commodity> {
    ...
    public IndexSpecification(IndexFilter filter) {
        this.filter = filter;
    ...
    private void findByModel(Root<Commodity> root, CriteriaQuery<?> query) {
        if (filter.getModelId() != null) {
            predicates.add(root.get("model").in(filter.getModelId()));
        }
    }

    private void findByDrive(Root<Commodity> root, CriteriaQuery<?> query) {
        if (filter.getDriveId() != null) {
            predicates.add(root.get("drive").in(filter.getDriveId()));
        }
    }
    ...
    public Predicate toPredicate(Root<Commodity> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {
        ...
        findByDrive(root, criteriaQuery);
        findByTransmission(root, criteriaQuery);
        findByModel(root, criteriaQuery);
